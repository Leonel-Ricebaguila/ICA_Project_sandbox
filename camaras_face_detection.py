...
class FaceDetector:
    ...
    def __init__(self):
        ...
        self.face_data = pd.DataFrame(columns=['timestamp', 'user_id', 'user_name', 'face_count', 'confidence'])
    ...
    def _load_models(self):
        ...
        try:
            ...
            try:
                ...
                # If models don't exist, use Haar cascades
    ...
    def detect_faces(self, frame: np.ndarray) -> Tuple[List[Tuple[int, int, int, int]], float]:
        ...
        if self.face_detector is not None:
            ...
            confidence = 0.8  # DNN confidence
        else:
            # Fallback to Haar cascades
            faces = self.detect_faces_haar(frame)
            confidence = 0.6  # Haar confidence
    ...
    def get_face_statistics(self) -> Dict:
        ...
        stats = {
            "total_detections": len(self.face_data),
            "unique_users": self.face_data['user_id'].nunique(),
            "avg_faces_per_detection": self.face_data['face_count'].mean(),
            "total_faces_detected": self.face_data['face_count'].sum(),
            "avg_confidence": self.face_data['confidence'].mean(),
            "detection_by_user": self.face_data.groupby('user_name')['face_count'].sum().to_dict()
        }
...
class EnhancedCameraManager:
    ...
    def __init__(self, server_url: str = SERVER_BASE_URL):
        ...
        self.log_to_server_enabled = True
    ...
    def log_to_server(self, event_type: str, user_id: int, user_name: str, data: Dict):
        ...
        try:
            ...
            # Send to server (you might want to add a logging endpoint)
    ...
    def display_video_streams(self):
        ...
        frame_count = 0
        detection_interval = 60  # Detect faces every 5 frames
